...
const AnalyticsPage = () => {
    ...
    // ИСПРАВЛЕНИЕ: Этот useEffect теперь будет вызван только один раз,
    // так как его зависимости (функции) больше не меняются при каждом рендере.
    useEffect(() => {
        fetchAnalyticsData();
        fetchInfluenceIndex();
    ...
    if (isLoading && (!analyticsData || !analyticsData.length)) {
        return <div className="page-layout"><div className="loading-state"><LoaderIcon/> Loading Analytics Data...</div></div>
    }

    return (
        <div className="page-layout analytics-layout">
